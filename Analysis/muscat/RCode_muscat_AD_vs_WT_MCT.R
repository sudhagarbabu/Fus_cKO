...
pbmc <- subset(pbmc, idents= c("AD", "WT"))
pbmc$genotype_num <- ifelse(grepl("AD", pbmc$genotype), 1, 0)
...
for (i in 1:length(celltypes)){
  ...
  outdir <- paste0("F:/Colab/Cambi_lab/Analysis/muscat/AD_vs_WT/MCT")
  dir.create(outdir, recursive = T)
  setwd(outdir)
  celltype <- gsub(" ","_",celltype)
  filename <- paste0(celltype,"_AD_vs_WT.csv")
